...
//Big O(n)
  void insertOrderly(const T& obj) //We will be sending object of type Record
	{	
		if(this->isEmpty())    //To check if the list is empty
		{
			this->insertFirst(obj); //if yes, then we insert just after the header
		}
		...
		{	
			if(p==this->getAfterLast()) //To check if we have reached the end i.e before trailer
			{
				this->insertLast(obj);
				return;
			}
			p = p -> getNext();      //To iterate
		}
		node->next=p;
		node->prev = p->prev;
...
// output operator
template<class T>
ostream& operator<<(ostream& out, const DoublyLinkedList<T>& dll)//Big O(n)
{
  ...
  else
  {
	...
	{								//So the loop goes on until current reaches the value of trailer		out<<current->obj<<" ";out<<current->getElem()<<" ";
		out<<current->getElem()<<" ";
		current = current->getNext(); //iterate
	}
  }

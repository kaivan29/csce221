...
void My_vec<T>::remove_at_rank(int r)   //7
{
 ...
 //In this insert at rank I was allocating memory to newptr, copying from ptr and then deleted ptr. But I forgot to copy back newptr to ptr. Was giving me a segmentation fault
/*	void insert_at_rank(int r, const T& elem)
	{
	if(r > size)                   //When we insert at a position greater than size we need to insert spaces and then insert the element. But fault segmentation is the error here.
	{
		T* newptr;
		if ( r>= capacity)
		{
	 	...
	 	for (int i = r-1; i >= size ; i--) //make a function shifting that shifts the elements and call the function multiple times whenever you want
		{
			newptr[i] = T();
		}
		newptr[r] = elem;
		size=r+1;//increase the size as an element is inserted
	}
	else if(r>=0&&r<=size){					//This works here!!
			if(size == capacity)
			{
				capacity = capacity * 2;
				T* newptr = new T[capacity];
